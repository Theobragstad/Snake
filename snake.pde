...
void setupCheckboxes() {
  for(int i=0; i < checkBoxes.length; i++) {
    if(i == 0) {checkBoxes[i] = new Checkbox(300, 169);}
    else if(i < 4 && i > 0) {checkBoxes[i] = new Checkbox(150+(i*20), 188);}   
    else if(i > 3 && i < 6) {checkBoxes[i] = new Checkbox(90+(i*20), 208);}
    else if(i > 5 && i < 8) {checkBoxes[i] = new Checkbox(30+(i*20), 228);}
    else if(i == 8) {checkBoxes[i] = new Checkbox(140, 248);}
  }
...
void displayOptions() {
  String[] choices = {"Don't let snake move through edges:",  "Secondary color:", "Number of fruits:", "Snake speed:", "Fruits move:"};
  fill(0);
  textSize(14);
...
void onlyOneBoxSelected() {
  if(checkBoxes[1].getChecked()) {checkBoxes[2].setChecked(false); checkBoxes[3].setChecked(false);}
  else if(checkBoxes[2].getChecked()) {checkBoxes[1].setChecked(false); checkBoxes[3].setChecked(false);}
  else if(checkBoxes[3].getChecked()) {checkBoxes[1].setChecked(false); checkBoxes[2].setChecked(false);}
  if(checkBoxes[4].getChecked()) {checkBoxes[5].setChecked(false);}
  else if(checkBoxes[5].getChecked()) {checkBoxes[4].setChecked(false);}
  if(checkBoxes[6].getChecked()) {checkBoxes[7].setChecked(false);}
  else if(checkBoxes[7].getChecked()) {checkBoxes[6].setChecked(false);}
}
...
void loadFruitImages() {
  int fruitCount = 1;
  if(checkBoxes[4].getChecked()) {fruitCount = 3;}
  else if (checkBoxes[5].getChecked()) {fruitCount = 5;}
...
int setSpeed() {
  int speed = 5;
  if(checkBoxes[6].getChecked()) {speed = 10;}
  else if(checkBoxes[7].getChecked()) {speed = 3;}
  return speed;
}
...
void move() {
 if(!paused) {
    for(int i=snakeLength; i >= 0; i--) { 
      if(i == 0) {
        if(headAngle == 0) {headX.set(0, headX.get(0)+20);} 
        else if(headAngle == 90) {headY.set(0, headY.get(0)-20);}
        else if(headAngle == 180) {headX.set(0, headX.get(0)-20);}
        else if(headAngle == 270) {headY.set(0, headY.get(0)+20);}  
      }
      else {
...
void display() {
  ...
  for(int i=1; i < 4; i++) {
    if(checkBoxes[i].getChecked()) {secondary=checkBoxes[i].getColor();}
  } 
  stroke(27,234,45);
  for(int i=0; i < snakeLength; i++) {
    if(i%2 == 0) {fill(27, 234, 45);}
    else if(i%2 == 1) {fill(secondary);}
...
void headImages() {
  if(headAngle == 0) {image(loadImage("right.jpg"), headX.get(0)+20, headY.get(0)+1);}
  else if(headAngle == 90) {image(loadImage("up.jpg"), headX.get(0)+1, headY.get(0)-18);}
  else if(headAngle == 180) {image(loadImage("left.jpg"), headX.get(0)-18, headY.get(0)+1);}
  else if(headAngle == 270) {image(loadImage("down.jpg"), headX.get(0)+1, headY.get(0)+20);}
}
...
void onEdge() {
  if(!checkBoxes[0].getChecked()) {
    if(headAngle == 180 && headX.get(0)+10 <= 0) {headX.set(0, width-20);}
    if(headAngle == 90 && headY.get(0)+10 <= 0) {headY.set(0, height-20);}   
    if(headAngle == 0 && headX.get(0) > width-10) {headX.set(0, 0);}
    if(headAngle == 270 && headY.get(0) > height-10) {headY.set(0, 0);}
  }
  else {
...
void gameOverScreen() {
  score = snakeLength-1; 
  if(setSpeed() == 10) {score /= 2;}
  else if(setSpeed() == 3) {score *= 2;}
  fill(0);
  stroke(0);
  textSize(30);
...
void keyPressed() {
    if(dead) {nameInput();}
    if(keyCode == SHIFT && !paused) {paused = true;}
    else if(keyCode == SHIFT && paused) {paused = false;}
    if(!paused) {
      if(keyCode == UP && headAngle != 270 && (headY.get(0)-20) != headY.get(1)) {headAngle = 90;}
      if(keyCode == DOWN && headAngle != 90 && (headY.get(0)+20) != headY.get(1)) {headAngle = 270;}
      if(keyCode == LEFT && headAngle != 0 && (headX.get(0)-20) != headX.get(1)) {headAngle = 180;}
      if(keyCode == RIGHT && headAngle != 180 && (headX.get(0)+20) != headX.get(1)) {headAngle = 0;}
    }
}
